...
class SubscriptionOrderService
{
    public static function Check(){
        ...
        $user_subscriptions = UserSubscriptionOrderDate::where('user_subscription_order_dates.order_date',$today)
        ->where('user_subscription_order_dates.status',1)
            ->join('user_subscriptions','user_subscriptions.id','=','user_subscription_order_dates.user_subscription_id')
            ->where('user_subscriptions.payment_status',3)
            ->where('user_subscriptions.status',2)
            ->select('user_subscriptions.*','user_subscription_order_dates.id as order_date_id')
            ->get();
            Log::debug($user_subscriptions);
            $seconds = 5;
        if ($user_subscriptions && (count($user_subscriptions) > 0)) {
            foreach ($user_subscriptions as $user_subscription){
                $jobs = new MemberShipOrder($user_subscription->order_date_id);
                $jobs->dispatch($user_subscription->order_date_id)->delay(Carbon::now()->addSeconds($seconds));
                $seconds += 5;
            }
        }
    }

    	        ...
    	        <div>
    	            <div class="x_content">                        
                        <div id="address-select">
                            {{ Form::open(['method' => 'get','route'=>'admin.order.invoice.membership.customer.proceed']) }}
  ...
  @section('script')
    <script>
        ...
        function checkMembership(user_id) {
            ...
            $.ajax({
                type:"GET",
                url:"{{url('admin/order/invoice/membership/check')}}"+"/"+user_id,
                success:function(data){
                    if (data) {
                        $("#membershipInfo").html(`<p style="color: red;font-weight: bold;">The Customer Already Have Membership<p>`);
                    } else {
                        $("#membershipInfo").html("");
